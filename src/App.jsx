...
const ParameterSlider = ({ label, type, options, trackClass, formData, setFormData, showErrors }) => {
  ...
  return (
    <div className={`input-group ${showErrors && !formData[type] ? 'invalid shake' : ''}`}>
      ...
      <div className="skin-slider-container">
        <div className="skin-slider-preview" style={{ background: 'rgba(255,255,255,0.03)', border: '1px solid rgba(198,142,23,0.1)' }}>          {type === 'tipo_pele' && <SkinIcon color={interpolateColor(localValue)} />}
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: '15px' }}>
          ...
          <input 
            ...
            className={`skin-range-slider ${trackClass || ''}`}
          />
...
const VisualSlider = () => {
  ...
  const getStepFromPath = (path) => {
    switch (path) {
      case '/': return 0;
      case '/identidade': return 1;
      case '/perfil': return 2;
      case '/upload': return 3;
      case '/processando': return 4;
      case '/analise-facial':
      case '/colorimetria':
      case '/matriz-de-poder':
      case '/dossie-final':
        return 5;
      default: return 0;
    }
  ...
  const steps = [
    { id: 0, label: 'INÍCIO', path: '/' },
    { id: 1, label: 'IDENTIDADE', path: '/identidade' },
    { id: 2, label: 'PERFIL', path: '/perfil' },
    { id: 3, label: 'FOTOS', path: '/upload' },
    { id: 4, label: 'PROCESSANDO', path: '/processando' },
    { id: 5, label: 'Dossiê', path: '/analise-facial' }
  ]

  return (
    <div className="funnel-progress-container animate-fade" style={{ marginBottom: '40px' }}>
      <div className="funnel-steps">
        {steps.map((s) => (
          <div key={s.id} className={`funnel-step ${currentStep >= s.id ? 'active' : ''}`}>
            <div className="step-dot">
              {currentStep > s.id ? (
                <svg viewBox="0 0 24 24" width="12" height="12" stroke="currentColor" strokeWidth="4" fill="none"><path d="M20 6L9 17l-5-5"/></svg>
              ) : (
...
const App = () => {
  ...
  useEffect(() => {
    if (location.pathname === '/processando') {
      const themes = [
  ...
  const renderWelcome = () => (
    <div className="animate-fade">
      ...
      <button className="btn-primary animate-slide-up btn-pulse" onClick={() => navigate('/identidade')}>INICIAR MAPA DE BELEZA</button>
    ...
    return (
      <div className="animate-fade">
        ...
        <div style={{ marginBottom: '40px', textAlign: 'center' }}>
          <h2 className="serif" style={{ textTransform: 'none', fontSize: '2rem' }}>Sua Identidade</h2>
          <p style={{ fontSize: '0.65rem', color: 'var(--text-muted)', letterSpacing: '2px', marginTop: '5px', opacity: 0.6 }}>PASSO 1 DE 2</p>
        </div>
  ...
  const renderQuizStep2 = () => {
    ...
    return (
      <div className="animate-fade">
        <VisualSlider />
        <div style={{ marginBottom: '40px', textAlign: 'center' }}>
          <h2 className="serif" style={{ textTransform: 'none', fontSize: '2rem' }}>Seu Perfil Visual</h2>
          <p style={{ fontSize: '0.65rem', color: 'var(--text-muted)', letterSpacing: '2px', marginTop: '5px', opacity: 0.6 }}>PASSO 2 DE 2</p>
        </div>
  ...
  const renderPhotos = () => {
    const handleGenerate = () => {
      if (userPhotos.selfie) {
        ...
        setAnalysisStep(0)
        setShowErrors(false)
        setTimeout(() => navigate('/analise-facial'), 25000)
      } else {
        setShowErrors(true)
      }
    }
  ...
  const renderProcessing = () => (
    <div className="processing-step animate-fade" style={{ textAlign: 'center', padding: '40px 20px' }}>
      <VisualSlider />
      <div style={{ marginBottom: '40px', textAlign: 'center' }}>
    ...
    return (
      <div className="animate-fade">
        ...
        <div className="animate-fade">
          ...
          <button className="btn-primary animate-slide-up" onClick={() => navigate('/colorimetria')}>CONTINUAR ANÁLISE • PRÓXIMA ETAPA</button>
        </div>
      </div>
    )
  }

  const renderResultColor = () => {
    const diagnostic = getDiagnostic();
    return (
      <div className="animate-fade">
        <VisualSlider />
        <div className="animate-fade">
          ...
          <button className="btn-primary animate-slide-up" onClick={() => navigate('/matriz-de-poder')}>REVELAR MINHA PALETA DE PODER</button>
        </div>
      </div>
    )
  }
    ...
    return (
      <div className="animate-fade">
        ...
        <div className="animate-fade">
          ...
          <button className="btn-primary animate-slide-up" style={{ marginTop: '30px' }} onClick={() => navigate('/dossie-final')}>REVELAR LOOKBOOK E DOSSIÊ FINAL</button>
        </div>
      </div>
    )
  }

  const renderDossierFinal = () => {
    return (
      <div className="animate-fade">
        <VisualSlider />
        <div className="animate-fade">
    ...
    <div className="app-container">
      <div className="main-layout" style={{ paddingTop: '0' }}>
        <Routes>
          ...
          <Route path="/identidade" element={renderQuizStep1()} />
          <Route path="/perfil" element={renderQuizStep2()} />
          <Route path="/upload" element={renderPhotos()} />
          <Route path="/processando" element={renderProcessing()} />
          <Route path="/analise-facial" element={renderResultFacial()} />
          <Route path="/colorimetria" element={renderResultColor()} />
          <Route path="/matriz-de-poder" element={renderResultMatrix()} />
          <Route path="/dossie-final" element={renderDossierFinal()} />
        </Routes>
